...
from wsgiref.headers importers
...
class StatusApp:
    ...
    def __call__(self, environ, start_response,ers=[]):
        if self.message:
           ers(headers).add_header('Content-type', 'text/plain')
        start_response(self.status,ers)
...
class Cling(object):
    ...
    def __call__(self, environ, start_response):
        ...
        if environ['REQUEST_METHOD'] not in ('GET', 'HEAD'):
           ers = [('Allow', 'GET,')]
            return self.method_not_allowed(environ, start_response,ers)
        ...
        if path.isdir(full_path):
            if full_path[-1] != '/' or full_path == self.root:
               ...
               ers = [('Location', location)]
                return self.moved_permanently(environ, start_response,ers)
            else:
    ...
    def _add_headers(self,ers, path, content_type):
