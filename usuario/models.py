...
"""
Esse é o modelo de dados para o pesquisador (ainda é um protótipo) ele é herdado
no arquivo forms.py que cria um modelo de formulario e envia para
views, para que possa ser passado para o cliente por meio de um template.  
"""
...
class Pessoa(AbstractUser):
    ...
    REQUIRED_FIELDS = ['nome', 'cpf', 'rg', 'rg_expedidor', 'lattes', 'foto', 'telefone',
                       'slug']  # Estes serao os dados passados para gerar o formulario
...
class Pesquisador(Pessoa):
    #  indentificacao = models.CharField('Indentificação de Pesquisador', max_length=20)
    vinculacao = models.CharField('Instituição de vinculação', max_length=100)
    ativo = True
...
class Bolsista(Pessoa):
    orientador = models.CharField('Orientador', max_length=100)
    ativo = False
